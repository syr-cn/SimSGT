...
def print_qm(accs, stds, categories, append_mean=False):
    category_line = ' '.join(categories)
    if append_mean:
        category_line += ' Mean'
...
def read_qm(args):
    ...
    for model, lines in model2lines.items():
        ...
        dataset_list = ['qm7', 'qm8', 'qm9']
        dataset_list = [dataset.lower() for dataset in dataset_list]
        rocs = [dataset2results.get(dataset, [0,0])[0] for dataset in dataset_list]
        stds = [dataset2results.get(dataset, [0,0])[1] for dataset in dataset_list]
        print_qm(rocs, stds, dataset_list, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='log_qm')
