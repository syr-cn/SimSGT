...
class MoleculeDataset_BRICS(InMemoryDataset):
    def __init__(self,
                 root,
                 #data = None,
                 #slices = None,
        ...
        super(MoleculeDataset_BRICS, self).__init__(root, transform, pre_transform,
                                                 pre_filter)
    ...
    def process(self):
        data_smiles_list = []
        data_list = []
        if self.dataset == 'zinc_standard_agent_brics':
            vocab = [x.strip("\r\n ") for x in open(f'{self.root}/clique.txt')]
            vocab = Vocab(vocab)
            ...
            for i in range(len(smiles_list)):
                ...
                try:
                    ...
                    if rdkit_mol != None:  # ignore invalid mol objects
                        ...
                        vocab_id = dict()
                        cliques, edges = brics_decomp(rdkit_mol)
                        for c in cliques:
                            cid = randint(0, int(1e9+7))
                            cmol = get_clique_mol(rdkit_mol, c)
                            csmiles = get_smiles(cmol)
                            wid = vocab.get_index(csmiles)
                            for i in c:
                                vocab_id[i] = (wid, cid)
                        vocab_id = [vocab_id[i] for i in range(len(rdkit_mol.GetAtoms()))]
