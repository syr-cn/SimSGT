...
def eval_chem(args, model, loader, label_mean, label_std, metric='rmse'):
    ...
    if metric == 'rmse':
        ...
        rmse_score = utils.mae(y_true, y_scores)
    else:
...
class GraphClf(nn.Module):
    def __init__(self, gnn, emb_dim, num_tasks, trans_pooling=None, args=None):
        ...
        if args.clf_norm == 'layer':
            ...
            self.norm = None
        # self.probe = nn.Linear(emb_dim, num_tasks)
        self.use_gnn2 = args.use_gnn2
        self.probe = nn.Sequential(
            nn.Linear(emb_dim, 2*emb_dim),
            nn.Dropout(args.probe_dropout),
            nn.LeakyReLU(),
...
def tuning_chem(args, train_dataset, valid_dataset, test_dataset, running_seeds):
    ...
    for runseed in tqdm(running_seeds):
        ...
        params = [
            ...
            {'params': model.probe.parameters(), 'lr': args.probe_lr, 'weight_decay': args.probe_decay}
        ]
        if model.norm:
...
def main():
    ...
    parser.add_argument('--probe_decay', type=float, default=0.0)
    parser.add_argument('--probe_dropout', type=float, default=0.0)
    parser.add_argument('--probe_lr', type=float, default=0.001)
    parser.add_argument('--use_gnn2', action='store_true', default=False)
    args = parser.parse_args()
    
    if args.complete_feature:
        if 'complete' not in args.tuning_dataset:
            args.tuning_dataset += 'complete'
    ...
    num_task_dict = {'qm7':1,'qm8':12,'qm9':12}
