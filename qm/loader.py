...
def atom_to_feature(atom):
    extra_feature = [
        one_of_k_encoding_unk(atom.GetTotalDegree(), allowable_features['possible_degree_list']) +
        one_of_k_encoding_unk(atom.GetFormalCharge(), allowable_features['possible_formal_charge_list']) +
        one_of_k_encoding_unk(atom.GetTotalNumHs(), allowable_features['possible_numH_list']) +
        one_of_k_encoding_unk(atom.GetNumRadicalElectrons(), allowable_features['possible_number_radical_e_list']) +
        one_of_k_encoding_unk(atom.GetHybridization(), allowable_features['possible_hybridization_list']) +
        one_of_k_encoding_unk(atom.GetIsAromatic(), allowable_features['possible_is_aromatic_list']) +
        one_of_k_encoding_unk(atom.IsInRing(), allowable_features['possible_is_in_ring_list'])
    ]
    return extra_feature

def atom_to_feature_index(atom):
    extra_feature = [
...
def edge_to_feature_vector(bond):
    extra_feat = one_of_k_encoding_unk(str(bond.GetStereo()), allowable_features['possible_bond_stereo_list']) + \
        one_of_k_encoding_unk(bond.GetIsConjugated(), allowable_features['possible_is_conjugated_list'])
    return extra_feat


def mol_to_graph_data_obj_simple_complete(mol):
    """
...
def mol_to_graph_data_obj_simple(mol, extra_feature=False):
    if extra_feature:
        return mol_to_graph_data_obj_simple_complete(mol)
    """
    Converts rdkit mol object to graph Data object required by the pytorch
    ...
    if len(mol.GetBonds()) > 0: # mol has bonds
        ...
        for bond in mol.GetBonds():
            ...
            edge_feature = [allowable_features['possible_bonds'].index(
                ...
                bond.GetBondDir())]
            extra_edge_feat = edge_to_feature_vector(bond)
            edges_list.append((i, j))
            edge_features_list.append(edge_feature)
            extra_edge_attr_list.append(extra_edge_feat)
            edges_list.append((j, i))
            edge_features_list.append(edge_feature)
            extra_edge_attr_list.append(extra_edge_feat)
        ...
        edge_attr = torch.tensor(np.array(edge_features_list),
                                 dtype=torch.long)
        extra_edge_attr = torch.FloatTensor(np.array(extra_edge_attr_list))
    else:   # mol has no bonds
        edge_index = torch.empty((2, 0), dtype=torch.long)
        edge_attr = torch.empty((0, num_bond_features), dtype=torch.long)
        extra_edge_attr = torch.empty((0, 8), dtype=torch.float)
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                try:
                    ...
                    continue
        elif self.dataset == 'zinc_standard_agent_complete':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path, sep=',', compression='gzip',
                                   dtype='str')
            smiles_list = list(input_df['smiles'])
            ...
            for i in range(len(smiles_list)):
                ...
                try:
                    ...
                    if rdkit_mol != None:  # ignore invalid mol objects
                        ...
                        # Chem.SanitizeMol(rdkit_mol,
                        #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
                        data = mol_to_graph_data_obj_simple_complete(rdkit_mol)
                        # manually add mol id
                        id = int(zinc_id_list[i].split('ZINC')[1].lstrip('0'))
                        data.id = torch.tensor(
                            [id])  # id here is zinc id value, stripped of
        ...
        elif self.dataset == 'qm7_complete':
            smiles_list, rdkit_mol_objs, labels = \
                _load_qm7_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
        ...
        elif self.dataset == 'qm8_complete':
            smiles_list, rdkit_mol_objs, labels = \
                _load_qm8_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
        ...
        elif self.dataset == 'qm9_complete':
            smiles_list, rdkit_mol_objs, labels = \
                _load_qm9_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, xt, y, smile_graph):
        ...
        for i in range(data_len):
            ...
            c_size, GCNData = smile_graph[smiles]

            # make the graph ready for PyTorch Geometrics GCN algorithms:
            GCNData.y = torch.FloatTensor([labels])
            GCNData.target = torch.unsqueeze(torch.LongTensor([target]), 0)
