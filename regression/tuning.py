...
class GraphClf(nn.Module):
    def __init__(self, gnn, emb_dim, num_tasks, trans_pooling='none'):
    ...
    def linear_probe(self, data):
        with torch.no_grad():
            ...
            if self.trans_pooling == 'cls':
                ...
                graph_rep = self.pool(node_representation, data.batch)
            graph_rep = graph_rep.detach()
        return self.linear(graph_rep)


def get_eval_loader(dataset, args):
    dataloader = DataLoaderSL(0, dataset, batch_size=256, shuffle=False, num_workers=0)
    if len(dataset) > 256:
...
def tuning_chem(args, train_dataset, valid_dataset, test_dataset, running_seeds):
    ...
    for runseed in tqdm(running_seeds):
        ...
        model = GraphClf(gnn, out_dim, args.num_tasks, args.trans_pooling)
...
def linear_probe_chem(args, train_dataset, valid_dataset, test_dataset, running_seeds):
    if args.force_noschedule:
        args.use_schedule = False
    train_eval_loader = get_eval_loader(train_dataset, args)
    val_loader = get_eval_loader(valid_dataset, args)
    ...
    for runseed in tqdm(running_seeds):
        ...
        optimizer = optim.Adam(
            model.linear.parameters(), lr=lr, weight_decay=args.decay)
        scheduler = StepLR(optimizer, step_size=30,
                           gamma=0.3) if args.use_schedule else None
        print(optimizer)
...
def main():
    ...
    if args.tuning_dataset is None:
        for dataset in datasets:
            ...
            if args.linear_probe:
                linear_probe_chem(args, train_dataset, valid_dataset,
