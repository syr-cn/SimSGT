...
class DataLoaderSL(torch.utils.data.DataLoader):
    ...
    def collate_fn(self, graphs):
        ...
        if self.mask_ratio > 0:
            ...
            if x_masked.shape[1] == 9:
                x_masked[mask_tokens] = torch.LongTensor([118, 4, 11, 11, 9, 5, 5, 0, 0])
            else:
            ...
            # mask_edge_tokens = torch.zeros(batch.edge_attr.shape[0], dtype=torch.bool)
            # mask_edge_tokens[torch.LongTensor(connected_edge_indices)] = True
            # batch.edge_attr[mask_edge_tokens] = torch.LongTensor([4, 0, 0])

        if self.add_self_loop:
            ## add self loop
            edge_index, _ = add_self_loops(batch.edge_index, num_nodes = batch.x.size(0))
            #add features corresponding to self-loop edges.
            self_loop_attr_dim = batch.edge_attr.shape[1]
            self_loop_attr = torch.zeros(batch.x.size(0), self_loop_attr_dim, dtype=torch.long)
            self_loop_attr[:,0] = 4 #bond type for self-loop edge
            edge_attr = torch.cat((batch.edge_attr, self_loop_attr), dim = 0)
            batch.edge_index = edge_index
            batch.edge_attr = edge_attr
