...
from pos_enc.pos_enc import eigvec_precompute, get_lap_decomp_stats, rw_precompute
...
class MoleculeDataset_Eig(InMemoryDataset):
    # Deprecated
    def __init__(self,
                 root,
                 #data = None,
        ...
        self.root = root
        self.multi_zinc = multi_zinc
        self.max_step = zinc_max_step

        super(MoleculeDataset_Eig, self).__init__(root, transform, pre_transform,
                                                 pre_filter)
    ...
    def multi_process_zinc(self):
        ...
        step_len = int(len(smiles_list)/self.max_step)

        data_smiles_list = []
        for step in range(self.max_step):
            data_list = []
            for i in range(step*step_len, (step+1)*step_len):
                s = smiles_list[i]
            ...
            target_path = self.processed_paths[0].replace('.pt',f'-{step}.pt')
            torch.save(data_list, target_path)
            del data_list


        data_smiles_series = pd.Series(data_smiles_list)
        data_smiles_series.to_csv(os.path.join(self.processed_dir,
                                                    f'smiles.csv'), index=False, header=False)
    ...
    def process(self):
        ...
        self.pre_transform = lambda data: eigvec_precompute(data)
        if self.pre_filter is not None:
            data_list = [data for data in data_list if self.pre_filter(data)]
...
class MoleculeDataset_Eig_v2(InMemoryDataset):
    def __init__(self,
                 ...
                 dataset='zinc250k',
                 empty=False,
                 multi_zinc=False, # deprecated
                 zinc_max_step=100,
                 args=None):
        """
        Molecule Dataset with Eigen Vectors precomputation.
        pad eigvects in self.get
        dataset saved to processed/geometric_data_processed.pt
        Eig precompute saved to processed/geometric_data_processed-eig.pt
        """
        self.dataset = dataset
        self.root = root
        self.multi_zinc = multi_zinc
        ...
        self.enable_eig = args.pe_type in ['signnet', 'lap', 'lap_v2', 'signnet_v2', 'signnet_v3']
        self.enable_rw = args.pe_type in ['rwse']

        super(MoleculeDataset_Eig_v2, self).__init__(root, transform, pre_transform,
                                                 pre_filter)
        self.transform, self.pre_transform, self.pre_filter = transform, pre_transform, pre_filter
        ...
        if not empty:
            ...
            if self.enable_eig:
                self.Eig_list = torch.load(self.processed_paths[0].replace('.pt', '-eig.pt'))
            if self.enable_rw:
                self.rw_list = torch.load(self.processed_paths[0].replace('.pt', '-rw.pt'))
    ...
    def get(self, idx):
        ...
        for key in self.data.keys:
            ...
            data[key] = item[s]
        if self.enable_eig:
            data.EigVals, data.EigVecs = self.Eig_list[idx]
            data.EigVals, data.EigVecs = get_lap_decomp_stats(
                evals=data.EigVals, evects=data.EigVecs,
                max_freqs=self.args.max_freqs,
                eigvec_norm=self.args.eigvec_norm
            )
        if self.enable_rw:
            data.pestat_RWSE = self.rw_list[idx]
        return data
    ...
    def process(self):
        ...
        if self.enable_eig:
            Eig_list = [eigvec_precompute(data, self.args) for data in data_list]
            torch.save(Eig_list, self.processed_paths[0].replace('.pt', '-eig.pt'))
        
        if self.enable_rw:
            rw_list = [rw_precompute(data, self.args) for data in data_list]
            torch.save(rw_list, self.processed_paths[0].replace('.pt', '-rw.pt'))
